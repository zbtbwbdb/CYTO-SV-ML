...
for i in $(seq 1 $sc_ln)
   do
       ...
       if [ -s ${main_dir}/out/${sample}/sv_caller_results/${sample}.${sv_caller}.${size_k}k.trs_tf.svtyped.vcf ]; then       
            awk '($1!~"#"){if ($5~"\\["){split($5,a,"["); $3=$1":"$2":"a[2]; print $0} else if ($5~"\\]"){split($5,a,"]"); $3=$1":"$2":"a[2]; print $0}}' ${main_dir}/out/${sample}/sv_caller_results/${sample}.${sv_caller}.${size_k}k.trs_tf.svtyped.vcf > ${main_dir}/out/${sample}/sv_caller_results/${sample}.${sv_caller}.${size_k}k.trs_tf.svtyped.vcf.tmp                
       fi
