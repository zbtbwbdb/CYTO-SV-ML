...
Rscript ${cyto_sv_ml_dir}/Pipeline_script/interface_docker.R -t OUTPUT_DIR+'/cyto_sv_ml/'+str(cohort_name)+'_'+sv_type+'_'+str(k)+'_trs_EXP -n  OUTPUT_DIR+'/cyto_sv_ml/'+str(cohort_name)+'_'+sv_type+'_'+str(k)+'_nontrs_EXP -i outdir+'/'+str(cohort_name)+'.sv.all.combine_all'
