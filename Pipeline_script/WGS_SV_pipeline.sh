...
for sample in $(cat ${main_dir}/${sample_id_list})
    do
        # SV Parliment2 run
        docker run --rm --privileged -v ${main_dir}/in/${sample}/:/home/dnanexus/in -v ${main_dir}/out/${sample}/:/home/dnanexus/out docker.io/dongwonlee/parliament2-sing:v0.12 --bam ${sample}.bam -r hg38/hs38.fasta --prefix ${sample} --filter_short_contigs --breakdancer --cnvnator --delly_deletion --delly_insertion --delly_inversion --delly_duplication

        # SV ChromoSeq run
        docker run --rm --privileged  -v ${main_dir}/:/scratch  --entrypoint /bin/sh zatawada/docker-basespace_chromoseq_v2:master -c '/usr/bin/java -Dconfig.file=/scratch/software/docker-${scratch}/${chromoseq_dir}/lsf/application.new.conf -jar /opt/cromwell-36.jar run -t wdl -i ${scratch}/${chromoseq_dir}/lsf/inputs.json.tmp ${scratch}/${chromoseq_dir}/workflow_files/Chromoseq.v17.wdl'
        ...
        for SV_database_name in gnomad_qc gnomad_ps 1000g cytoatlas cosmic donor_g
            do
                if [ -e ${main_dir}/SV_database/${SV_database_name}.nontrs.gz ]; then
                    python sv_database_mapping.py -i ${main_dir}/out/${sample}/vcf_out/${sample}.sv.all.notrs -t ${main_dir}/SV_database/${SV_database_name}.nontrs.gz -d 1000 -p 0.7 -o ${main_dir}/out/${sample}/vcf_out/${sample}.sv.all.notrs_${SV_database_name} 
                fi
                if [ -e ${main_dir}/SV_database/${SV_database_name}.trs.gz ]; then
                ...
                python sv_db_tf.py ${main_dir}/out/${sample}/vcf_out/${sample}.sv.all.trs ${SV_database_name} 
                python sv_db_tf.py ${main_dir}/out/${sample}/vcf_out/${sample}.sv.all.notrs ${SV_database_name}   
            done
