...
def sv_id_tf(line):
 ...
 #   print(str(item[1])+":"+str(sv_end))        
    ...
    elif (id_mode=='c' or  id_mode=='sc') and 'chr' not in item[2]:
        sv_id=item[0]+':'+item[1] +':'+sv_end+':'+sv_chr2 +':'+sv_type+":"+item[2]        
    elif 'chr' in item[2] or re.findall('DEL|DUP|INV|INS|TRA|BND|CNV',item[2]) or re.findall('del|dup|inv|ins|tra|bnd|cnv',item[2]) or ':' in item[2]:
        sv_id=item[2]
    else:
        sv_id=item[0]+':'+item[1] +':'+sv_end+':'+sv_chr2 +':'+sv_type+":"+item[2]
    item[2]=sv_id
