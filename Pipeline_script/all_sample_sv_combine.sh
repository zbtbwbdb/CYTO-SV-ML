...
for i in $(seq 1 $sm_ln)
  do
    ...
    if (( i==1 )); then
        awk -v a="$sample" '{if (FNR==1) {print $0"\tsample_id"} else {print $0"\t"a}}' ${sample_file} > ${main_dir}/out/${sample_all}/${sample_all}.sv.all.combine_all.tmp
    else
        awk -v a="$sample" '(FNR>1){print $0"\t"a}' ${sample_file} >> ${main_dir}/out/${sample_all}/${sample_all}.sv.all.combine_all.tmp      
        fi
  done
ls -a ${main_dir}/out/${sample_all}/${sample_all}.sv.all.combine_all.tmp  
sed 's% %\t%g'  ${main_dir}/out/${sample_all}/${sample_all}.sv.all.combine_all.tmp > ${main_dir}/out/${sample_all}/${sample_all}.sv.all.combine_all
