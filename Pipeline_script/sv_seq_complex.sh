...
python ${cyto_sv_ml_dir}/Pipeline_script/sv_vcf_bed_tf.py ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all
awk 'FNR==NR{a[$1];b[$1]=$2;next}{c=b[$1]-150 ; if (($2>=150)&&($2<=c)) {$2=$2-150; $3=$2+150; print $1"\t"$2"\t"$3"\t"$5"\t"$6} else if ($2>c) {$2=c-150;$3=c+150; print $1"\t"$2"\t"$3"\t"$5"\t"$6} else if ($2<150){$2=1;$3=300; print $1"\t"$2"\t"$3"\t"$5"\t"$6}}' ${cyto_sv_ml_dir}/reference/hg38_chromosome_size.txt ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed | sed 's% %\t%g' > ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.bpst
awk 'FNR==NR{a[$1];b[$1]=$2;next}{$1=$4; $4=$1; c=b[$1]-150 ; if ($3>=c) {$3=c+150;$2=c-150; print $1"\t"$2"\t"$3"\t"$5"\t"$6} else {$2=$3-150; $3=$3+150;  print $1"\t"$2"\t"$3"\t"$5"\t"$6}}' ${cyto_sv_ml_dir}/reference/hg38_chromosome_size.txt ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed | sed 's% %\t%g' > ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.bpend
...
for bp in bpst bpend
    do
       ...
       ${py27_dir}/bedtools getfasta -fi ${cyto_sv_ml_dir}/reference/hg38/hs38.fasta -bed ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.${bp} -fo ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.${bp}.fa.out
      ...
      # rm ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.${bp}.fa.out.tmp   
       ...
       perl ${cyto_sv_ml_dir}/software/SeqComplex/profileComplexSeq.pl ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.${bp}.fa.out.lc
       kz --fasta < ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.${bp}.fa.out > ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.bed.${bp}.fa.out.kz
