...
for SV_database_name in uwstl_s 
    do
        ...
        if [ -s ${main_dir}/out/${sample}/${sample}/sv_caller_results/${sample}.${SV_database_name}.trs ]; then
            ${py27_dir}/python ${cyto_sv_ml_dir}/Pipeline_script/sv_bnd_database_mapping.py ${main_dir}/out/${sample}/sv_caller_results/${sample}.${SV_database_name}.trs ${main_dir}/out/${sample}/${sample}.${size_k}k.sv.all.trs ${SV_database_name}_1000 
        else
