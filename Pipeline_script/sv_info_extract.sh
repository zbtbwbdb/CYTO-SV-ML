...
for svtype in breakdancer cnvnator delly.deletion delly.duplication delly.inversion ichnorcnv manta 
    do
      if [ -s ${main_dir}/out/${sample}/sv_caller_results/${sample}.${svtype}.vcf.10k.sv_info.sim ]; then 
          echo $svtype "ok" && date       
          if [ "${n}" == 0 ]; then
              awk 'FNR==NR{a[$3];b[$3]=$0;next} {if ($2 in a) {print $0"\t"b[$2]} else {print $0}}' ${main_dir}/out/${sample}/sv_caller_results/${sample}.${svtype}.vcf.10k.sv_info.sim ${main_dir}/out/${sample}/${sample}.10k.sv.all.sv_id_mapping.tmp0 > ${main_dir}/out/${sample}/${sample}.10k.sv.all.sv_id_mapping.tmp1
              n=1
          else
              awk 'FNR==NR{a[$3];b[$3]=$0;next} {if ((FNR!=1)&&($2 in a)) {print $0"\t"b[$2]} else {print $0}}' ${main_dir}/out/${sample}/sv_caller_results/${sample}.${svtype}.vcf.10k.sv_info.sim ${main_dir}/out/${sample}/${sample}.10k.sv.all.sv_id_mapping.tmp0 > ${main_dir}/out/${sample}/${sample}.10k.sv.all.sv_id_mapping.tmp1    
          fi
        else
...
info_check=$(awk '{print NF}'  ${main_dir}/out/${sample}/${sample}.10k.sv.all.sv_id_mapping.tmp0 | awk '!a[$1]++' | wc -l | awk '{print $1}')
...
for svtype in breakdancer cnvnator delly ichnorcnv manta 
  do
      if [ -s ${main_dir}/out/${sample}/${sample}.${svtype}.svtyped.vcf.10k.sv_info.sim ]; then             
          ...
          if [ -s ${main_dir}/out/${sample}/sv_caller_results/${sample}.${svtype}.vcf.10k.sv_info.sim ]; then                     
              cp ${main_dir}/out/${sample}/sv_caller_results/${sample}.${svtype}.vcf.10k.sv_info.sim ${main_dir}/out/${sample}/${sample}.${svtype}.svtyped.vcf.10k.sv_info.sim.tmp
              if [ "${n}" == 0 ]; then
...
info_check=$(awk '{print NF}' ${main_dir}/out/${sample}/${sample}.10k.sv.all.sv_id_mapping.all_info | awk '!a[$1]++'  | wc -l | awk '{print $1}')
if (($info_check>1)); then echo "sv_info missing !!!"; fi    