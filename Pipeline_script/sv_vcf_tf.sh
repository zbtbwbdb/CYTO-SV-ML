...
for sv_caller in  breakdancer cnvnator delly.deletion delly.duplication delly.inversion ichnorcnv manta       
    do
        ...
        awk 'FNR==NR{a[$1];c[$1]=$2;next}{split($3,d,":"); e=b[1]":"b[2]":"b[3]":"b[4]":"b[5]; if (($1!~"#")&&(e in a)) {$3=c[e]; print $0}}' ${main_dir}/out/${sample}/sv_caller_results/${sample}.${sv_caller}.vcf.s.id ${main_dir}/out/${sample}/sv_caller_results/${sample}.${sv_caller}.vcf.re_id | sed 's% %\t%g' >   ${main_dir}/out/${sample}/sv_caller_results/${sample}.${sv_caller}.vcf.re_id.s
